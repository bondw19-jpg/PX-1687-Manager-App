...
import {
  ...
  Printer, Plus, Trash2, X, Edit3, ChevronRight, ListChecks, GripVertical,
} from 'lucide-react';
...
function CustomChecklistModal({ checklist, onSave, onClose }) {
  ...
  const addItem = () => {
    ...
    if (!text) return;
    setItems(prev => [...prev, { id: `item_${Date.now()}`, text, checked: false }]);
    setNewText('');
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-t-2xl w-full animate-slide-up flex flex-col" style={{ height:'85dvh', maxHeight:'85dvh', overflow:'hidden' }}>
        ...
        <div className="modal-body p-4 space-y-4">
          ...
          {/* Add item */}
          <div>
            <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Add Items</label>
            <div className="flex gap-2">
              <input
                ref={inputRef}
          ...
          {items.length > 0 && (
            <div className="space-y-1.5">
              <p className="text-xs font-semibold text-gray-500">{items.length} item{items.length !== 1 ? 's' : ''}</p>
              {items.map((item, idx) => (
    ...
    <div className="space-y-3">
      ...
      {customChecklists.length === 0 ? (
        <div className="bg-white rounded-xl p-8 flex flex-col items-center text-gray-400">
          ...
          <p className="text-sm font-medium">No custom checklists yet</p>
      ...
      <div className="desktop-page-content p-4 lg:p-0 space-y-4">
        ...
        <div className="flex bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          <button onClick={() => setMainTab('custom')}
            ...
            <ListChecks size={16} /> My Checklists
          </button>
        </div>
