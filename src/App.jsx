...
// ── FirestoreInit: attaches real-time listeners once on app start ─────────────
function FirestoreInit() {
  const initFirestore        = useAppStore(s => s.initFirestore);
  const migrateLocalToFirestore = useAppStore(s => s.migrateLocalToFirestore);

  useEffect(() => {
    // 1. Attach real-time Firestore listeners → populates all collections live
    initFirestore();
    // 2. One-time migration: push existing localStorage data into Firestore
